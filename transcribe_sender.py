...
def main(page: ft.Page):
    ...
    def text_size_callback(_):
        for list_item in transcription_list.controls:
            list_item.size = int(text_size_dropdown.controls[0].value)
        transcription_list.update()
    ...
    def clear_transcription_callback(_):
        ...
        transcription_text_from_list.value = create_transcription_text_widget()
        transcription_list.update()
        transcription_text_from_list.update()

    def create_transcription_text_widget():
        combined_text = '\n'.join([item.controls[0].value for item in transcription_list.controls])
    ...
    model_dropdown = ft.Dropdown(
        options=[
            ...
            ft.dropdown.Option('large', text="Large (High Quality)"),
            ft.dropdown.Option('large-v2', text="Large v2 (Ultra Quality)"),
        ],
    ...
    transcription_text_from_list = ft.Text("", selectable=True, color=ft.colors.BLACK87)
    ...
    page.add(
        ...
        ft.Container(
            content=transcription_text_from_list,
            padding=ft.padding.only(left=15, right=45, top=5),
            expand=True,
            bgcolor=ft.colors.WHITE70, width=page.window_width,
        ),
    )
    ...
    while True:
        ...
        if currently_transcribing and audio_model and not data_queue.empty():
            ...
            def create_copy_function(text_to_copy):
                return lambda _: copy_to_clipboard(text_to_copy)

            def copy_to_clipboard(text):
                # Implement the logic to copy `text` to clipboard
                # This depends on your environment; you might use pyperclip or tkinter
                pyperclip.copy(text)
            ...
            if now > next_transcribe_time:
                ...
                result = audio_model.transcribe(audio_normalised, language=language, task=task, beam_size=beam_size,
                ...
                # Function to create a custom item layout using Row
                def create_custom_item(text, text_size, color, on_button_click, index_in_list):
                    text_object = ft.Text(value=text, selectable=True, size=text_size, bgcolor=color,
                                          width=int(0.7 * page.window_width))
                    copy_button = ft.ElevatedButton(
                        text="\n\nCopy\n\n",
                        on_click=on_button_click,
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=5)),
                        width=int(0.10 * page.window_width)
                    )

                    # Create a Row layout with text_object and copy_button
                    custom_row = ft.Row(controls=[text_object, copy_button], width=page.window_width, wrap=True)
                    return custom_row

                # Usage in your existing code
                if not phrase_complete and transcription_list.controls:
                    # Assuming the last control in transcription_list is a Row
                    last_row = transcription_list.controls[-1]
                    # Update text in the first control of the Row
                    last_row.controls[0].value = text
                    # Update button click event in the second control of the Row
                    last_row.controls[1].on_click = create_copy_function(text)

                elif not transcription_list.controls or (
                        transcription_list.controls and transcription_list.controls[-1].controls[0].value):
                    custom_item = create_custom_item(
                        text,
                        int(text_size_dropdown.value),
                        color,
                        create_copy_function(text),
                        index_in_list
                    )
                    index_in_list += 1
                ...
                transcription_list.update()
                transcription_text_from_list.value = create_transcription_text_widget()
                transcription_text_from_list.size = int(text_size_dropdown.value)
                transcription_text_from_list.update()
